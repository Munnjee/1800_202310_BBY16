...
// Function for employee to leave review for employer
function writeReview() {
  let Level = document.getElementById("level").value;
  let Description = document.getElementById("description").value;
  let RedoGig = document.querySelector('input[name="redoGig"]:checked').value;
  let WorkWithEmployerAgain = document.querySelector(
    'input[name="workWithEmployerAgain"]:checked'
  ).value;
  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      currentUser.get().then((userDoc) => {
        ...
        gigListingRef.get().then((gigListingDoc) => {
...
// Function for employer to leave review for employee
