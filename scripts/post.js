...
postForm.addEventListener("submit", (event) => {
  ...
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
      ...
      const compensationValue = parseFloat(
        postForm.compensation.value.replace(/\$/g, "")
      );
      db.collection("giglisting")
        .add({
          ...
          flexDate: postForm.flexibleDate.checked
            ? postForm.flexibleDate.value
            : "",
          time: postForm.time.value,
          flexTime: postForm.flexibleTime.checked
            ? postForm.flexibleTime.value
            : "",
        })
        .then((doc) => {
